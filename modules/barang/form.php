						...
						<form role="form" class="form-horizontal" action="modules/barang/proses.php?act=insert" method="POST">
							<div class="box-body">
								<?php 
									...
									$query_id=mysqli_query($mysqli, "SELECT RIGHT(kd_barang,6) as kode FROM tb_barang ORDER BY kd_barang DESC LIMIT 1") or die('Ada kesalahan pada query tampil data barang : '.mysqli_error($mysqli));
									...
									<div class="form-group">
										...
										<div class="col-sm-5">
											<select class="chosen-select" name="kategori" data-placeholder="--Pilih--" autocomplete="off" required>
												<option value=""></option>
												<option value="Poliner">Poliner</option>
												<option value="Bernike">Bernike</option>
											</select>
										</div>
									</div>
