<?php 
	...
	// jika user sdh login maka jalankan perintah utk insert ,update  dan delete
			...
			if(isset($_POST['kd_transaksi'])){
				...
				$exp				= explode('-',$tanggal);
				$tanggal_keluar		= $_POST['tanggal_keluar'];//$exp[2]."-".$exp[1]."-".$exp[0];
			...
			elseif (isset($_POST['detail'])) {
				...
				$query_detail_keluar = mysqli_query ($mysqli, "SELECT p.nama_barang, p.kategori, dk.kd_transaksi, dk.jumlah_keluar, dk.harga FROM detail_keluar dk INNER JOIN tb_barang p ON p.kd_barang = dk.kd_barang WHERE dk.kd_transaksi = '$kd_transaksi'");
