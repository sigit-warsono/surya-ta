<?php 
	...
	// jika user sdh login maka jalankan perintah utk insert ,update  dan delete
		...
		if (isset($_POST['kd_transaksi'])) {
			...
			$query_detail_masuk = mysqli_query ($mysqli, "SELECT p.nama_barang, p.kategori, dm.kd_transaksi, dm.jumlah_masuk, dm.harga FROM detail_masuk dm INNER JOIN tb_barang p ON p.kd_barang = dm.kd_barang WHERE dm.kd_transaksi = '$kd_transaksi'");
