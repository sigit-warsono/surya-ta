            ...
            <tbody>
                <?php 
 		 				...
 		 				// jika data tdk ada
 		 					...
 		 					// tampilkan data
							$sql = mysqli_query($mysqli,"delete from temp") or die('Ada kesalahan pada query tampil Transaksi : '.mysqli_error($mysqli));
 		 					$bobot = 1;
 		 					$peramalan = 0;
 		 					$total_bobot = 0;
							$h=0;
							...
							for ($i=1;$i<=$periode;$i++) {
 		 					...
 		 					while ($data=mysqli_fetch_assoc($query)){$h++;
								$lim1=$h-1-$periode;
								
								//echo "SELECT jumlah_keluar FROM detail_keluar WHERE kd_barang = '$kd_barang' AND MONTH(tanggal_keluar) BETWEEN '$bln_awal' AND '$bln_akhir' GROUP BY MONTH(tanggal_keluar), YEAR(tanggal_keluar) limit ".$lim1.",".$periode."";
								...
								$sql = mysqli_query($mysqli," insert into temp VALUES ($data[bulan1],$data[tahun],$hasil/$bagi)") or die('Ada kesalahan pada query tampil Transaksi : '.mysqli_error($mysqli));
 		 						...
 		 						echo "<tr>
										...
										<td style='padding-left:10px;' width='155' height='13' valign='middle'><div align=right>".pt($hasil/$bagi)."</div></td>
							...
							echo "<tr>
 		 								<td width='120' height='13' align='center' valign='middle' colspan='5'><div align=justify>
										Hasil Prediksi</div></td>
							...
							$bobot = 1;
 		 					for ($i=1; $i <= $jbul; $i++) { 
								...
								$sql = mysqli_query($mysqli," insert into temp VALUES ($bulan_depans,$tahun_depans,$hasil/$bagi)") or die('Ada kesalahan pada query tampil Transaksi : '.mysqli_error($mysqli));
	 		 					echo "<tr>
	 		 								<td width='120' height='13' align='center' valign='middle'><div align=justify>".$namaBulan[$bulan_depans]." </div></td>
											<td width='120' height='13' align='center' valign='middle' ><div align=center>".$tahun_depans."</div></td>	
											<td style='padding-left:10px;' width='155' height='13' valign='middle'><div align=right>-</div></td>
											<td style='padding-left:10px;' width='155' height='13' valign='middle'><div align=right>$bobot</div></td>
